...
# Fixed Password hashing functions
def verify_password(plain_password, hashed_password):
    """Verify password with bcrypt or fallback to sha256"""
    try:
        # Check if it's our fallback hash format
        if hashed_password.startswith("sha256$"):
            _, salt, stored_hash = hashed_password.split("$")
            computed_hash = hashlib.sha256((plain_password + salt).encode()).hexdigest()
            return secrets.compare_digest(computed_hash, stored_hash)
        else:
            # Try bcrypt verification with proper encoding and truncation
        ...
        # Final fallback - compare raw (for development only)
        return False

def get_password_hash(password):
    """Hash password with bcrypt, fallback to sha256 if bcrypt fails"""
    try:
        # Try bcrypt first with proper encoding and truncation
        password_bytes = password.encode('utf-8')
        if len(password_bytes) > 72:
            password_bytes = password_bytes[:72]
        return pwd_context.hash(password_bytes)
    except Exception as e:
        logger.warning(f"Bcrypt hashing failed, falling back to sha256: {e}")
...
def authenticate_user(db, email: str, password: str):
    ...
    if not user:
        # To prevent timing attacks, use a dummy verification
